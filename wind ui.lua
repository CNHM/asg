...
warn(string.format("[ WindUI.Creator ] 加载自定义资源失败 '%s': %s",M,tostring(P)))
L:Destroy()
...
if not N then
warn("[ WindUI.Creator ]  '"..identifyexecutor().."' 不支持URL图片，错误: "..O)
...
ai"您正在受到速率限制，请等待20秒后重试";
return false
else
ai"服务器返回了无效状态码，请稍后重试";
return false
end
end
...
elseif aD.StatusCode==429 then
return false,("您正在受到速率限制，请等待20秒后重试")
else
return false,("服务器返回了无效状态码，请稍后重试")
end
end
...
local an=am.Key_Information.Notes or"未知原因"

return false,"验证失败: "..an
end
else
...
warn("[Pelinda Ov2.5] HTTP请求未成功，代码： "..tostring(ak.StatusCode).." Message: "..ak.StatusMessage)
return false,"HTTP请求失败: "..ak.StatusMessage
end
else
...
return false,"该卡密关联的HWID已变更，请通过我们的机器人重置密钥"

elseif(ah.code=="KEY_INCORRECT")then
return false,"卡密错误或已被删除！"
else
...
local aA=240
local aB=false
local aC=ae("获取卡密","key",nil,"Secondary",ax.Frame)
...
error("未知开关组件类型: "..tostring(ai.Type))
end
...
Title="提示",
Content="您是否要关闭此脚本？关闭后将无法再次打开此脚本",
Buttons={
{
Title="还是算了",
